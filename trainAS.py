...
# data_train1=pd.read_csv(Path("../path/Dataset1/DatasetAS"))
# data_train2=pd.DataFrame()
# data_val1=pd.read_csv(Path("../path/Dataset1/DatasetAS"))
# data_val2=pd.DataFrame()

# if there are two datasets to analyze

# data_train1=pd.read_csv(Path("path/Dataset1/DatasetAS"))
# datas_train2=pd.read_csv(Path("path/Dataset2/DatasetAS"))  
# data_train=pd.concat([data_train1,data_train2], axis=0, ignore_index=True)

# data_val1=pd.read_csv(Path("path/Dataset1/DatasetAS"))
# datas_val2=pd.read_csv(Path("path/Dataset2/DatasetAS"))  
...
class firescardataset():
    def __init__(self, dataset, ss1, ss2, ss3, ss4, mult=1, transform=None):
        ...
        for i in range(ss1,ss2):
            self.seglabels.extend([Path("path/to/Firescars/Dataset1/") / dataset.loc[i,"FireScar_tif"]])
            self.imgfiles.extend([Path("path/to/Img/Post/Dataset1/") / dataset.loc[i,"ImgPosF"]])
            self.imgprefiles.extend([Path("path/to/Img/Pre/Dataset1/") / dataset.loc[i,"ImgPreF"]])
        for i in range(ss3,ss4):
            self.seglabels.extend([Path("path/to/Firescars/Dataset2/") / dataset.loc[i,"FireScar_tif"]])
            self.imgfiles.extend([Path("path/to/Img/Post/Dataset2/") / dataset.loc[i,"ImgPosF"]])
            self.imgprefiles.extend([Path("path/to/Img/Pre/Dataset2/") / dataset.loc[i,"ImgPreF"]])
        ...
        for i, batch in progress:
            ...
            for j in range(y.shape[0]):                                       
                ...
                if (np.sum(output_binary[j][0]) != 0 and
                    ...
                    train_ious.extend([z])
                    TP_train.extend([(output_binary.squeeze()*y.cpu().detach().numpy().squeeze()).sum()])
                    FN_train.extend([((output_binary.squeeze()==0) & (y.cpu().detach().numpy().squeeze()==1)).sum()])
                    FP_train.extend([((output_binary.squeeze()==1) & (y.cpu().detach().numpy().squeeze()==0)).sum()])
                    dicec_train_acc.extend([dice2d(output_binary,y.cpu().detach().numpy())])
          ...
          # derive IoU values
            ...
            for k in range(y.shape[0]):
                ...
                if (np.sum(output_binary[k][0]) != 0 and 
                    ...
                    val_ious.extend([z])
                    TP_eval.extend([(output_binary.squeeze()*y.cpu().detach().numpy().squeeze()).sum()])
                    FN_eval.extend([((output_binary.squeeze()==0) & (y.cpu().detach().numpy().squeeze()==1)).sum()])
                    FP_eval.extend([((output_binary.squeeze()==1) & (y.cpu().detach().numpy().squeeze()==0)).sum()])
                    dicec_eval_acc.extend([dice2d(output_binary,y.cpu().detach().numpy())])
